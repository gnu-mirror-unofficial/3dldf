...
@q *** (3) command --> picture_vector_variable PLUS_ASSIGN numeric_expression. @>
...
@<Define rules@>=
@=command: picture_vector_variable PLUS_ASSIGN numeric_expression@>@/
{
   ...
   if (entry == static_cast<Id_Map_Entry_Node>(0))
      {
          ...
          cerr_strm << thread_name << "ERROR!  In `yyparse()', rule "
                    << "`command --> picture_vector_variable PLUS_ASSIGN numeric_expression':"
                    << endl
		    << "`picture_vector_variable' is invalid.  "
                    << "Not incrementing counter.  Will try to continue.";
