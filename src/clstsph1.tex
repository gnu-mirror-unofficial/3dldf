...
The two models are provided in two versions, white on black and black
...
The second model uses ``panels''. While the left and
right sides of the sections appear to be curved, they are in fact made
up of straight lines. The sections should be folded on the horizontal
lines (latitude) and again attached using the tabs. The result is an
irregular polyhedron approximating a sphere.

The ``globes'' on pages 10--11 illustrate how the positions of the stars on the spherical biangles
and panels were derived.
\vfil\eject
\endgroup
